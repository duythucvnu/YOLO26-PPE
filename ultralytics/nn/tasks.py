...
def parse_model(d, ch, verbose=True):
    ...
    base_modules = frozenset(
        {
            ...
            DWConv, Focus, BottleneckCSP, C1, C2, C2f, C3k2, RepNCSPELAN4, ELAN1, ADown, AConv, SPPELAN, C2fAttn,
            C3, C3TR, C3Ghost, torch.nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, PSA, SCDown,
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            m = m(*args)
            c2 = m.channel # Lấy list channel [0, 0, P3, P4, P5]
            
        else:
            c2 = ch[f]

        # Khởi tạo hoặc đóng gói module
        t = str(m)[8:-2].replace("__main__.", "")  # module type
        if isinstance(c2, list):
            is_backbone = True
            m_ = m
        ...
        # Lưu các layer index cần lấy output (được cộng 4 nếu là backbone)
        ...
        # Cập nhật danh sách channels đầu ra
        if isinstance(c2, list):
            ch.extend(c2)
            for _ in range(5 - len(ch)):
                ch.insert(0, 0) # Padding zero cho đủ 5 layers P1->P5
        else:
            ch.append(c2)
